...
func (cc *CLI) TestWriteBlock(args []string) {
	// Initialize a block
	bk := block.InitBlock(12, "Welcome to Blockchain Wallet", []byte{1, 2, 3, 4})
...
func (cc *CLI) SendFundsTransaction(
	...
	fmt.Printf("\nBefore sending funds the balances for %s and %s are as follows:\n", from, to)
	fmt.Printf("Acct: %s Value: %d\n", to, cc.GetTotalValueForAccount(to))
	fmt.Printf("Acct: %s Value: %d\n", from, cc.GetTotalValueForAccount(from))

	// Get the total balance for `from`
	tot := cc.GetTotalValueForAccount(from)
	if tot < amount {
		return nil, fmt.Errorf("Insufficient funds")
	}
	// Get the list of output transactions
	oldOutputs := cc.GetNonZeroForAccount(from)
	for _, out := range oldOutputs {
		delete(cc.BlockIndex.FindValue.AddrIndex, fmt.Sprintf("%s", out.Account))
	}
	// Create a new transaction
	tx = transactions.NewEmptyTx(memo, from)
	tx.Input, _ = transactions.CreateTxInputsFromOldOutputs(oldOutputs)
	txOut, _ := transactions.CreateTxOutputWithFunds(to, amount)
	transactions.AppendTxOutputToTx(tx, txOut)
	change := tot - amount
	if change > 0 {
		txOut2, _ := transactions.CreateTxOutputWithFunds(from, change)
		transactions.AppendTxOutputToTx(tx, txOut2)
	}

	fmt.Printf("Sent %s $%d from %s\n", to, amount, from)
	return
}
