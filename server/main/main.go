...
// --test-read-block
// --test-write-block
// --test-send-funds From To Amount x x x x
...
func getRespHandlerShutdown(cc *cli.CLI) HandlerFunc {
	return func(www http.ResponseWriter, req *http.Request) {
		...
		addr, fail := GetParam(cc, www, req, "addr")
		if fail {
			return
		}
		...
		// gCfg := config.GetGlobalConfig()
		isValid, err := cc.ValidateSignature(addr, signature, msg)
		if err != nil {
			return
		}
		if !isValid {
...
func getRespHandlerSendFundsTo(cc *cli.CLI) HandlerFunc {
	return func(www http.ResponseWriter, req *http.Request) {
		...
		rv := cc.SendFundsJSON(from, to, amount, addr, signature, msg, memo)
		io.WriteString(www, rv)
	}
}
